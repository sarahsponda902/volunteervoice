    ...
    def index
     ...
     def update
       @review = Review.find(params[:id])
        ...
        params[:review][:body] = "<b>Update #{Time.now.to_date.strftime("%m/%e/%Y").gsub(/^0/, '')}:</b> "+RedCloth.new( ActionController::Base.helpers.sanitize( params[:review][:body] ), [:filter_html, :filter_styles, :filter_classes, :filter_ids] ).to_html + "<br />"+@review.body
