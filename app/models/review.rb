...
def resize_review_photos
  if !photo.blank?
    ...
    if image[:width] > 600
      scaling_width_percent = (image[:width] / 600)*100.round - 5
      image = image.sample(scaling_width_percent.to_s + "%")
    end  
    if image[:height] > 300
      scaling_height_percent = (image[:height] / 300)*100.round - 5
      image = image.sample(scaling_height_percent.to_s + "%")
    end
    self.photo = image
  end
  if !photo2.blank?
