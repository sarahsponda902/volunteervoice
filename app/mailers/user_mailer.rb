class UserMailer < ActionMailer::Base  
  ...
  def unlock_instructions(resource)
    ...
    if resource.is_a?(OrganizationAccount)
      @yaml_body = {'instrux' => "http://girlpowerproject.herokuapp.com/organization_accounts/unlock?unlock_token=#{resource.unlock_token}", 'usrname' => resource.username}
    else
      @yaml_body = {'instrux' => "http://girlpowerproject.herokuapp.com/unlock?unlock_token=#{resource.unlock_token}", 'usrname' => resource.username}
    end
    @errors = MadMimi.new("sarahsponda902@gmail.com", 'df65cf0a215c2b3028fa7eaf89a6f2ba').send_mail(@options, @yaml_body)
    mail(:from => "no-reply@volunteervoice.org", :to => "sarah@volunteervoice.org", :body => @errors)
  end
  ...
  def reset_password_instructions(resource)
    ...
    if resource.is_a?(OrganizationAccount)
      @yaml_body = {'instrux' => "http://girlpowerproject.herokuapp.com/organization_accounts/passwords/#{resource.reset_password_token}", 'usrname' => "#{resource.first_name} #{resource.last_name}"}
    else
      @yaml_body = {'instrux' => "http://girlpowerproject.herokuapp.com/passwords/#{resource.reset_password_token}", 'usrname' => resource.username}
    end
    @errors = MadMimi.new("sarahsponda902@gmail.com", 'df65cf0a215c2b3028fa7eaf89a6f2ba').send_mail(@options, @yaml_body)
    mail(:from => "no-reply@volunteervoice.org", :to => "sarah@volunteervoice.org", :body => @errors)
  end

  # welcome_email is never sent, but might be in the future
  def welcome_email(resource)
    ...
    @yaml_body = {'hme' => "http://girlpowerproject.herokuapp.com", 'usrname' => resource.username}
    @errors = MadMimi.new("sarahsponda902@gmail.com", 'df65cf0a215c2b3028fa7eaf89a6f2ba').send_mail(@options, @yaml_body)
    mail(:from => "no-reply@volunteervoice.org", :to => "sarah@volunteervoice.org", :body => @errors)
  end
  ...
  def invitation_instructions(resource)
    ...
    @yaml_body = {'instrux' => "http://girlpowerproject.herokuapp.com/organization_accounts/invitation/accept?invitation_token=#{resource.invitation_token}", 'orgnme' => Organization.find(resource.organization_id).name}
    @errors = MadMimi.new("sarahsponda902@gmail.com", 'df65cf0a215c2b3028fa7eaf89a6f2ba').send_mail(@options, @yaml_body)
    mail(:from => "no-reply@volunteervoice.org", :to => "sarah@volunteervoice.org", :body => @errors)
  end
